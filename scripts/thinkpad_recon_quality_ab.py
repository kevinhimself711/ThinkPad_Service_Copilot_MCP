...
def main() -> int:
    ...
    for i, p in enumerate(sample, 1):
        ...
        img = imgs[0]
        t0 = time.time()
        steps_a = vs.reconstruct_steps(model_a, [img], p)
        lat_a.append(time.time() - t0)
        t1 = time.time()
        steps_b = vs.reconstruct_steps(model_b, [img], p)
        lat_b.append(time.time() - t1)
        a_steps_tot += len(steps_a)
        b_steps_tot += len(steps_b)
        a_empty += not steps_a
        b_empty += not steps_b
